#!/usr/bin/env python3
"""
Ingest "Secrets Unsealed" PDFs (paragraph + chapter layers) into TinyOwl's ChromaDB.
...
Notes:
  - Uses the same BGE-large model as the rest of the theology DB for consistency
  - Backs up existing collections (if safety helper available) when --recreate is passed
  - Writes JSON chunk lists to domains/theology/chunks for transparency
"""
...
DEFAULT_SRC = Path.home() / "Downloads" / "Secrets Unsealed"
...
FRONT_MATTER_PATTERNS = (
    ...
    re.compile(r"library of congress", re.I),
)
...
def chapterize(raw: str) -> List[str]:
    ...
    if blocks:
        return blocks
    # Fallback: aggregate paragraphs into ~3k char chunks
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Ingest Secrets Unsealed PDFs into paragraph/chapter collections")
    ap.add_argument("--src", type=str, default=str(DEFAULT_SRC), help="Path to 'Secrets Unsealed' folder")
    ...
    print(f"🦉 Secrets Unsealed ingest: {len(files)} files from {src_dir}")
    ...
    if args.recreate:
        ...
        col_par = recreate(client, "secrets_paragraphs", bge_ef)
        col_chp = recreate(client, "secrets_chapters", bge_ef)
        par_existing = 0
        chp_existing = 0
    else:
        def get_or_create_resume(name: str):
            ...
            return col, 0
        col_par, par_existing = get_or_create_resume("secrets_paragraphs")
        col_chp, chp_existing = get_or_create_resume("secrets_chapters")
    ...
    # Persist chunk JSON lists
    chunks_dir = ROOT / "domains/theology/chunks"
    chunks_dir.mkdir(parents=True, exist_ok=True)
    (chunks_dir / "secrets_unsealed_paragraphs.json").write_text(
        __import__('json').dumps(all_par_docs, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    (chunks_dir / "secrets_unsealed_chapters.json").write_text(
        __import__('json').dumps(all_chp_docs, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    print(f"\n💾 Saved chunk lists: secrets_unsealed_paragraphs.json ({len(all_par_docs)}), secrets_unsealed_chapters.json ({len(all_chp_docs)})")
    ...
    print("\n📊 Final counts:")
    print(f"  secrets_paragraphs: {col_par.count()}")
    print(f"  secrets_chapters  : {col_chp.count()}")
    print("\n✅ Secrets Unsealed ingest complete.")
