#!/usr/bin/env python3
"""
Ingest 3ABN PDFs (paragraph + chapter layers) into TinyOwl's ChromaDB.

Collections:
  - threeabn_paragraphs
  - threeabn_chapters

Follows the same structure as SOP/Secrets/Amazing Facts.
"""
...
DEFAULT_SRC = Path.home() / "Downloads" / "3ABN"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Ingest 3ABN PDFs into paragraph/chapter collections")
    ap.add_argument("--src", type=str, default=str(DEFAULT_SRC), help="Path to '3ABN' folder")
    ...
    print(f"🦉 3ABN ingest: {len(files)} files from {src_dir}")
    ...
    if args.recreate:
        ...
        col_par = recreate(client, "threeabn_paragraphs", bge_ef)
        col_chp = recreate(client, "threeabn_chapters", bge_ef)
        par_existing = 0
        chp_existing = 0
    else:
        def get_or_create_resume(name: str):
            ...
            return col, 0
        col_par, par_existing = get_or_create_resume("threeabn_paragraphs")
        col_chp, chp_existing = get_or_create_resume("threeabn_chapters")
    ...
    (chunks_dir / "threeabn_paragraphs.json").write_text(
        __import__('json').dumps(all_par_docs, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    (chunks_dir / "threeabn_chapters.json").write_text(
        __import__('json').dumps(all_chp_docs, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    print(f"\n💾 Saved chunk lists: threeabn_paragraphs.json ({len(all_par_docs)}), threeabn_chapters.json ({len(all_chp_docs)})")
    ...
    print("\n📊 Final counts:")
    print(f"  threeabn_paragraphs: {col_par.count()}")
    print(f"  threeabn_chapters  : {col_chp.count()}")
    print("\n✅ 3ABN ingest complete.")
