...
class TinyOwlChat:
    ...
    def print_header(self):
        ...
        print(f"{Colors.HEADER}{Colors.BOLD}🦉 TinyOwl Interactive Chat{Colors.ENDC}")
        print(f"{Colors.DIM}Your personal theological knowledge assistant{Colors.ENDC}")
        print(f"{Colors.HEADER}{'='*60}{Colors.ENDC}")
        print()
...
Note: RAG (knowledge base search) is currently disabled, so I'm responding based on general knowledge only."""
    ...
    def handle_command(self, command):
        ...
        if command == "/models":
            ...
            try:
                choice = input(f"\\nEnter model number (1-{len(self.available_models)}): ").strip()
                idx = int(choice) - 1
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # Display response
                print(f"{Colors.BOLD}🦉 TinyOwl:{Colors.ENDC} {response}")
                print()
